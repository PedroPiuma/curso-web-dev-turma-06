...
function senioridadeCrescente(devs) {
    let aux
    for (let i = 0; i < devs.length; i++) {
        for (let j = 0; j < devs.length; j++) {
            if (devs[i].senioridade === 'estagio' && (devs[j].senioridade === 'junior' || devs[j].senioridade === 'pleno' || devs[j].senioridade === 'senior')) {
                aux = devs[i]
                devs[i] = devs[j]
                devs[j] = aux
            } else if (devs[i].senioridade === 'junior' && (devs[j].senioridade === 'pleno' || devs[j].senioridade === 'senior')) {
                aux = devs[i]
                devs[i] = devs[j]
                devs[j] = aux
            } else if (devs[i].senioridade === 'pleno' && devs[j].senioridade === 'senior') {
                aux = devs[i]
                devs[i] = devs[j]
                devs[j] = aux
            }
